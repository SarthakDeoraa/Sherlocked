...
export default function PlayPage() {
  ...
  // Fetch current question for the user's team
  const {
    data: question,
    isLoading,
    isError,
  ...
  // Fetch hints for the current question
  const {
    data: hintsData,
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (data) => {
      setAnswer("");
      queryClient.invalidateQueries({ queryKey: ["play", "current-question"] });
      if (data?.correct) {
        toast.success("Correct answer!", { style: { background: '#22c55e', color: 'white' } });
      } else if (data?.correct === false) {
        toast.error("Try again, wrong answer.", { style: { background: '#ef4444', color: 'white' } });
      }
    },
    onError: (error: any) => {
      // Handle rate limit error
      if (error?.response?.status === 429) {
        toast("Wait 5 seconds after every try.", { style: { background: '#fbbf24', color: 'black' } });
      }
    },
  });
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-center text-white font-vonca text-2xl flex-1">
                {isLoading ? "Loading..." : isError ? "Error" : question ? `LEVEL - ${question.level}` : "No Question"}
              </CardTitle>
              {question && (
                <HintPopup
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-6 flex flex-col items-center">
              {isLoading ? (
                ...
                <div className="text-red-400">{error instanceof Error ? error.message : "Failed to load question."}</div>
              ) : question ? (
                <>
