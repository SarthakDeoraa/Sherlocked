...
export default function AdminDashboard() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axios.get("/api/admin/question/list", {
  ...
  const createQuestionMutation = useMutation({
    mutationFn: async (payload: NewQuestion) => {
  ...
  const deleteQuestionMutation = useMutation({
    mutationFn: async (questionId: string) => {
      const res = await axios.delete("/api/admin/question/delete", {
        data: { questionId },
        headers: getAuthHeader(),
      });
      return res.data;
              ...
              <Card className="bg-white/10 backdrop-blur-md border-none shadow-xl rounded-2xl">
                ...
                >
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewQuestion((q) => ({ ...q, points: e.target.value }))
                    }
                    className="bg-white/20 text-gray-900 placeholder:text-gray-400"
                    required
                  />
        ...
        <div className="w-full max-w-3xl space-y-8 mt-4">
          {isLoading ? (
            ...
            questions.map((q: Question) => (
              <Card
                key={q.id}
                className="bg-white/10 backdrop-blur-md border-none shadow-lg rounded-2xl"
              >
                <CardHeader>
                ...
                <CardContent>
                  ...
                  {q.imageUrl && (
                    <a
                      ...
                      className="text-blue-300 underline mb-2 block"
                    >
                      View Image
                    </a>
                  )}
                  <div className="mt-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {q.hints.map((hint: Hint) => (
                        <div
                          key={hint.id}
                          className="flex items-center justify-between bg-yellow-50/10 rounded px-3 py-2"
                        >
                          <span
                            className={
                              hint.isEnabled
                    ...
                    >
                      <Input
                        ...
                        value={newHint[q.id] || ""}
                        onChange={(e) =>
                          setNewHint((h) => ({ ...h, [q.id]: e.target.value }))
                        }
